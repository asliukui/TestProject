...
def get_tab_tcount(tab_index=4):
    if tab_index == 0:  # 查自己
        return f"select count(1) as tcount from table"
...
def get_field_true(index,row, df, tab_index=4):
    if tab_index == 0:  # 查自己
        return f"select count(1) as tcount from table where nvl({df.iloc[0, 1]},'') !=''"
    elif tab_index == 1:  # 1张中间表 统计数据量
        return f"select count(1) as tcount from table t,tableA a where t.{df.iloc[0, 1]} = a.{df.iloc[0, 20].replace(',','')} and t.{row.iloc[1]} = a.{row.iloc[16].replace(',','')} "
    elif tab_index == 2:  # 2张中间表
        if index==0:
            return (f"select count(1) as tcount from table t,("
                f"select {df.iloc[0, 20].replace(',','')} from tableA "
                f"union  all "
                f"select {df.iloc[0, 20].replace(',','')} from tableB) a where  t.{row.iloc[1]} = a.{row.iloc[16].replace(',','')}")
        else:
            return (f"select count(1) as tcount from table t,("
                f"select {df.iloc[0, 20].replace(',','')},{row.iloc[20].replace(',','')} from tableA "
                f"union  all "
                f"select {df.iloc[0, 20].replace(',','')},{row.iloc[20].replace(',','')} from tableB) a where t.{df.iloc[0, 1]} = a.{df.iloc[0, 16].replace(',','')} and t.{row.iloc[1]} = a.{row.iloc[16].replace(',','')}")
    elif tab_index == 3:  # 3张中间表
        if index == 0:
            return (f"select count(1) as tcount from table t ,("
                    ...
                    f"where t.{row.iloc[1]} = a.{row.iloc[16].replace(',', '')}")
        else:
            return (f"select count(1) as tcount from table t ,("
                f"select {df.iloc[0, 20].replace(',','')},{row.iloc[20].replace(',','')} from tableA "
                f"union  all "
                ...
                f"where t.{df.iloc[0, 1]} = a.{df.iloc[0, 16].replace(',','')} and t.{row.iloc[1]} = a.{row.iloc[16].replace(',','')}")
    elif tab_index == 4:
        return f"select 0 as tcount from dual"
    else:
        return f"select '' as tcount from dual"
    ...
    for index, row in df.iterrows():
        ...
        if df.iloc[index, 4] == common.PRIMARY_KEY or (df.iloc[index, 4] == "Y"):  # 1.主键唯一;2.执行语句
            ...
            df.loc[index, df.columns[
                sbean.col_num_sql1]] = f"select count (1) tcount from table where nvl({df.iloc[index, 1]},'') !='' group by {df.iloc[index, 1]} "
            df.loc[index, df.columns[sbean.col_num_sql2]] = get_tab_tcount(0)
        elif (df.iloc[index, 5] == "否") or (df.iloc[index, 5] == "N"):  # 1.测试意图,判断非主键，不可为空 2.执行sql语句
            df.loc[index, df.columns[sbean.col_num_intent1]] = common.test_intent(row.iloc[2], row.iloc[1],
                                                                                  common.flagArr[3])
            df.loc[index, df.columns[
            sbean.col_num_sql1]] = f"select count(1) as tcount from table where nvl({df.iloc[index, 1]},'') !=''"  # sql判定值不为空
            df.loc[index, df.columns[sbean.col_num_sql2]] = get_tab_tcount(0)
            ...
            if sbean.col_num_intent1 == 25:  # 3段映射
                df.loc[index, df.columns[sbean.col_num_sql2]] = get_tab_tcount(1)
            elif sbean.col_num_intent1 == 37:
                df.loc[index, df.columns[sbean.col_num_sql2]] = get_tab_tcount(2)
            elif sbean.col_num_intent1 == 49:
                df.loc[index, df.columns[sbean.col_num_sql2]] = get_tab_tcount(3)
